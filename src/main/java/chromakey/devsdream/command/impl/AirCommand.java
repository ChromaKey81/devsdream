...
public class AirCommand {
    ...
    private static int setAir(CommandSource source, Collection<? extends Entity> targets, int amount)
        ...
        if (i == 0) {
            ...
            if (targets.size() == 1) {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.air.set.success.single",
                        targets.iterator().next().getDisplayName(), amount), true);
            } else {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.air.set.success.multiple",
    ...
    private static int increaseAir(CommandSource source, Collection<? extends Entity> targets, int amount)
            throws CommandSyntaxException {
        ...
        if (list.isEmpty()) {
            ...
            if (list.size() == 1) {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.air.add.success.single",
                        list.iterator().next().getDisplayName(), amount), true);
            } else {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.air.add.success.multiple",
