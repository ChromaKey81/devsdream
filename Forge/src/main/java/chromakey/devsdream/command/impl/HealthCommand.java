...
public class HealthCommand {
    ...
    private static int healEntity(CommandSource source, Collection<? extends Entity> targets, float amount)
        ...
        if (list.isEmpty()) {
            ...
            if (list.size() == 1) {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.health.add.success.single",
                        list.iterator().next().getDisplayName(), amount), true);
            } else {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.health.add.success.multiple",
    ...
    private static int setEntityHealth(CommandSource source, Collection<? extends Entity> targets, float amount)
            throws CommandSyntaxException {
        List<Entity> list = Lists.newArrayListWithCapacity(targets.size());
        ...
        if (list.isEmpty()) {
            ...
            if (list.size() == 1) {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.health.set.success.single",
                        list.iterator().next().getDisplayName(), amount), true);
            } else {
                source.sendFeedback(new TranslationTextComponent("commands.devsdream.health.set.success.multiple",
                        list.size(), amount), true);
            }
